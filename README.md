# То, что узнал в книге "JAVA. Эффективное программирование", 3-е издание, Джошуа Блох

## Описание

Это мой заметки того, что я узнал из
...
Гасите свой первый порыв предоставления открытого конструктора классу, предпочитайте фабричные методы.
...
Статические служебные и сингтоны непригодны для классов, поведение которых параметризованно лежащим в их основам ресурсам
...
- Финализаторы непредсказуемы из-за запоздалой финализации, часто опасны и в общем случае не нужны
...
- Финализаторы являются серьезной проблемой безопасности: они открывают ваш класс для атак финализаторов (https://habr.com/ru/articles/586994/)
...
Отложенная инициализация используется в String::hashCode
...
Возможно потребуется убрать `final`, чтобы клонировать объект.
...
При добавлении компонент-значения к классу, реализующему интерфейс Comparable не расширяйте его, а напишите новый независимый класс содержащий экземпляр первого класса
...
- Неизменяемые объекты потокобезопасны и не нужна синхронизация и обеспечивают атомарность.
- Неизменяемые объекты можно использоваться совместно.
- Можно кешировать часто запрашиваемые объекты.
...
У данного подхода мало недостатков, один из них `SELF problem` (https://coderanch.com/t/670687/java/wrapper-class-suitable-callback-framework), остальные менее существенные.
...
Метод, который вызывает методы, могущие быть перекрытыми, содержит описание этих вызовов в конце документирующего комментария. 
Это описание является отдельным разделом спецификации, именуемым "Требования реализации и генерируемым дескриптором: @implSpec.
Пример @implSpec в AbstractCollection::remove или AbstractList::removeRange
...
В присутствии дефолтных методов существующие реализации интерфейсов могут компилироваться без ошибок или предупреждений, но сбоить во время выполнения(?).
...
- Анонимный класс(anonymous class)
  Нельзя инстанцировать, за исключением точки объявления. Нельзя применить тест instanceof или выполнить иные действия, требующие имени класса.
  Нельзя объявить анонимный класс для реализации нескольких интерфейсов или для расширения класса и реализации интерфейса одновременно.
- Локальный класс(local class)
(https://stackoverflow.com/questions/478804/advantage-of-local-classes-java/479937#479937 https://stackoverflow.com/questions/3247654/when-should-you-use-a-local-class-in-java)
...
`List != List <Object>` https://stackoverflow.com/questions/7885920/java-difference-between-list-and-listobject#:~:text=List%20is%20not%20compatible,List%20s%20of%20all%20parameters.
...
Heap pollution – ситуация, когда эта защита не срабатывает, и переменная параметризованного типа хранит в себе объект, параметризованный другим типом.
```java
...
Пример: https://stackoverflow.com/questions/23942528/generic-singleton-factory
...
[system.err] Exception in thread "main" java.lang.ClassCastException: [Ljava.lang.Object; incompatible with [Ljava.lang.String;
```
...
Так же возможно написать свой процессор аннотаций:
https://www.baeldung.com/java-annotation-processing-builder
https://habr.com/ru/companies/e-legion/articles/206208/
...
Для лямбда-выражения идеально одна строка кода, а три это разумный максимум.
...
Лямбда-выражения, как и экземпляр анонимного класса нельзя надежно сериализовать и десериализовать. 
...
осторожностью ([Item 21](#47-проектируйте-интерфейсы-для-потомков-item-21)). 
...
Всегда аннотируйте ваши функциональные интерфейсы с помощью аннотации `@FunctionalInterface`.

Не делайте перегрузку метода ([Item 52](#84-перезагружайте-методы-разумно-item-52)), которые принимают разные
...
Stream API вычисляются отложено: вычисления не начинаются до тех пор, пока не будет вызвана завершающая(терминальная) операция, и
...
удобочитаемости конвейеров потоков. 
...
Будьте аккуратны при использовании потоков для обработки значений типа char:
...
*Входные данные не должны зависеть от изменяемого состояния* - из лямбда-выражения можно прочесть только
...
Пример простейшего коллектора отображения ```toMap(keyMapper, valueMapper)```:

```java
private static final Map<String, Operation> stringToEnum = Stream.of(values()).collect(toMap(Object::toString, e -> e));
```

Это простая форма ```toMap``` идеальна, если каждый элемент в потоке отображается на уникальный ключ. Иначе
```IllegalStateExeption```. В данной ситуации можно воспользоваться трехаргументной разновидностью ```toMap```
...
Чтобы сделать конвейерные операции в Stream API многопоточными, нужно вызвать метод `parallel()`.
...
В качестве грубой оценки, количество элементов в потоке, умноженное на количество строк кода, выполняемых
для каждого элемента, должно составлять по крайней мере сто тысяч. (Lea, Doug. When to use parallel streams. 
...
Так же для защиты, нужно возвращать копии изменяемых внутренних объектов:
...
- Избегайте длинных списков параметров. 4 параметра - это разумный максимум. Особенно вредны длинные последовательности
...
До Java 5 все примитивные типы являлись "совершенно иными" по отношению ко всем ссылочным типам, но при наличии автоматической
упаковки это становится неверным, что приводит к реальным проблемам.
...
Решением этой проблемы будет: ```list.remove((Integer) i)```

Добавление лямбда-выражений и ссылок на методы в Java 8 увеличивает возможности путаницы при перегрузке. Например:

```java
new Tread(System.out::println).start(); // Компилируется

ExecutorService exec = Executors.newCachedTreadPool();
exec.submit(System.out::println); // Не компилируется
```
```System.out::println``` является неточной ссылкой на метод и что "определенные выражения аргументов, которые
содержат неявно типизированные лямбда-выражения или неточные ссылки на метод, игнорируются тестами на 
применимость, поскольку их значение не может быть определено до тех пор, пока не будет выбран целевой тип"
...
Если вы считаете, что выделение массива нулевой длины вредит произвоводительности, то можете возвращать один и тот 
же массив нулевой длины многократно, потому что все массивы нулевой длины являются неизменяемыми. 
...
Соглашения о том как писать java-doc: https://www.oracle.com/technical-resources/articles/java/javadoc-tool.html

Если класс поддерживает сериализацию, следует документировать его сериализированную
...
Для большенства применений следует выбирать генератор случайных чисел TreadLocalRandom. 
...
```java
Long sum = 0L; // ~3,7 slowly than long!
...
`ArrayList<Son> subscribers = new ArrayList<>();`

Хорошо - использование интерфейсов в качестве типов:

`List<Son> subscribers = new ArrayList<>();`
...
Хотя эта программа не более чем игрушка, продемонстрированная методика оказывается достаточной чтобы переделать в 
тестирующую программу для обобщенных множеств, которая проверяет правильность определений реализации Set.
...
Инструмент, который заслуживает особого упоминания, -jmp, который является не профайлером, а каркасом тонкой проверки
производительности, который предоставляет беспрецедентно подробную информацию о производительности java-кода.
...
Имена пераметров обычно состоят из одной буквы, чаще всего это один из 5 вариантов: T - для произвольного типа, 
E - для типа элемента коллекции, K и V для типов ключа и значения отображения и X - для исключения. Тип возвращаемого
значения функции, как правило, именуется как R. Последовательность произвольных типов может быть T,U,V или T1, T2, T3.
